...
| Ventas       | pedido simple de una línea                 |                       Operativo | `createOrder`, `POST /api/orders`                                              | impuesto demo 19 % y estados no centralizados                                  |
...
| Auditoría    | eventos visibles del recorrido             |               Operativo en demo | eventos locales y `outbox_events`                                              | evento servidor no es atómico con la operación y carece de metadatos completos |
...
| API          | corte vertical Fastify                     |       Operativo como referencia | build TypeScript y smoke Compose                                               | monolito en un archivo; no apto para internet                                  |
...
| transición de pedido válida         |   solo pago pendiente → pagado |                       condicional disperso | máquina explícita y matriz completa                               |
...
| evento auditable completo           |                        parcial |                                    parcial | actor, correlación, causación, versión, sanitización y atomicidad |
