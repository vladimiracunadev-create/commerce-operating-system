...
| Inventario   | recepción, saldo, reserva y disponibilidad |                       Operativo | motor local, constraints `stock`, cancelación y expiración                      | sin ledger inmutable, ajustes o transferencias                                 |
...
| Pagos        | aprobación y descuento de stock            |                        Simulado | `payOrder` idempotente, `/pay/mock`, prueba concurrente                         | no mueve dinero ni integra proveedor real                                     |
...
| PostgreSQL   | esquema y consistencia transaccional       |       Operativo como referencia | `001_schema.sql`, actualización aditiva `002`, pruebas de rollback/concurrencia | runner e historial general de migraciones pendientes                          |
...
| disponibilidad = física - reservada |                             sí |                   sí, protegida por constraints | ledger de movimientos pendiente                                  |
| reserva no supera disponibilidad    |              sí en flujo único | `FOR UPDATE`, update guardado y constraint | otras rutas futuras deberán conservar el contrato                 |
| pago descuenta/libera una vez       |          idempotente por clave | idempotente y probado bajo concurrencia      | proveedor real y conciliación pendientes                          |
...
| cancelar/expirar libera una vez     |                 sí, con pruebas |              sí, con pruebas concurrentes | política operativa de TTL pendiente                               |
| reembolso ≤ pago                    |                             no |                                         no | implementación y pruebas                                          |
| documento mock pagado y único       |        sí, reintento devuelve igual | índice, bloqueo y prueba concurrente     | proveedor DTE real pendiente                                      |
